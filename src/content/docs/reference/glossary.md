...
Terms used across this guide, in one line each where possible. For full detail on any tool named here, ask Claude to call `get_context` or `get_help`, or check the [official repo](https://github.com/busportals/portals-mcp).

- **`apply_operations`** — MCP tool that applies a batch of targeted, atomic changes to a room, always against a freshly downloaded snapshot; rejects stale or partial writes. See [Gotchas](/reference/gotchas/).
...
- **`PORTALS_BRIDGE_PORT`** — Environment variable that overrides the runtime bridge's default port (`3099`) if it's already in use. See [Troubleshooting](/reference/troubleshooting/).
